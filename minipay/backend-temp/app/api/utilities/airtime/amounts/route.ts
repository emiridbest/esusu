...
export async function GET(req: NextRequest) {
  try {
    ...
    // Fetch airtime amounts for the operator
    const amountsResponse = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/operators/${providerId}/airtime-amounts`,
      {
        headers: {
    ...
    return Response.json(amountsResponse.data.content);
    
  } catch (error: any) {
    console.error('Error fetching airtime amounts:', error);
    
    return Response.json(
      { error: error?.response?.data?.message || error?.message || 'Failed to fetch airtime amounts' },
