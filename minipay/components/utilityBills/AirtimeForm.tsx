...
export default function AirtimeForm() {
  ...
  useEffect(() => {
    const getNetworks = async () => {
      if (watchCountry) {
        setIsLoading(true);
        form.setValue("network", ""); // Reset network selection
  ...
  useEffect(() => {
    if (watchAmount) {
      setSelectedPrice(Number(watchAmount));
    } else {
      setSelectedPrice(0);
    }
  }, [watchAmount]);
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      try {
        ...
        setIsProcessing(false); // Stop processing on verification error
        return;
      } finally {
        setIsVerifying(false); // Ensure verifying state is reset
      }
