...
async function validatePayment(validation: PaymentValidation): Promise<{ isValid: boolean; error?: string }> {
  try {
    ...
    const transferDataHexLength = 2 + (4 + 32 * 2) * 2; // "0x" + 4-byte selector + two 32-byte args
    let decoded;
    try {
      decoded = transferInterface.parseTransaction({ data: rawData });
    } catch (primaryError) {
      // Some wallets (and our frontend) append referral metadata to the calldata.
      // Try trimming the extra bytes and decode again.
      const trimmedData = rawData?.length > transferDataHexLength
        ? rawData.slice(0, transferDataHexLength)
        : rawData;
      try {
        decoded = transferInterface.parseTransaction({ data: trimmedData });
        console.debug('Decoded token transfer after trimming referral metadata', {
          originalLength: rawData?.length,
          trimmedLength: trimmedData.length
        });
      } catch (secondaryError) {
        console.error('Failed to decode token transfer calldata', {
          originalLength: rawData?.length,
...
export async function POST(request: NextRequest) {
    try {
        ...
        // Validate required fields - customerEmail is optional for Farcaster
        if (!country || !providerId || !customerId || !amount) {
            return NextResponse.json(
                { success: false, error: 'Missing required fields: country, providerId, customerId, amount' },
                { status: 400 }
            );
        }
            ...
            transaction = await TransactionService.createTransaction({
                ...
                utilityDetails: {
                    ...
                    metadata: { 
                        ...
                        customerPhone: customerPhone?.trim() || customerEmail?.trim() || '', 
                        requestReference 
                    }
                }
            });
